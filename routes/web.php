...
Route::get('/check-config', function() {
    return [
        ...
        'TELEGRAM_CONFIG' => config('services.telegram.token') ? 'SET *****' . substr(config('services.telegram.token'), -4) : 'NULL',
        'TELEGRAM_ENV' => env('TELEGRAM_BOT_TOKEN') ? 'SET *****' . substr(env('TELEGRAM_BOT_TOKEN'), -4) : 'NULL',
    ];
});
